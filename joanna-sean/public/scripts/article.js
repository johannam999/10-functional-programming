...
(function (module){
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map(ele => ele.body.match(/\b\w+/g).length).reduce( (acc, curr) =>
      acc + curr)
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(ele => ele.author).reduce(function(names, name) {
